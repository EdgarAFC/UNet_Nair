...
class ONEPW_Dataset(Dataset):
    def __init__(self, data, onepw_img, enh_img):
        '''
        ...
        self.train_enh_img = enh_img

        self.images = sorted(os.listdir(self.train_data))
        self.onepw_images = sorted(os.listdir(self.train_onepw_img))
        self.enh_images = sorted(os.listdir(self.train_enh_img))
  
    #regresar la longitud de la lista, cuantos elementos hay en el dataset
    def __len__(self):
        if self.onepw_images is not None:
          assert len(self.images) == len(self.onepw_images)== len(self.enh_images), 'not the same number of images ans enh_images'
    ...
    def __getitem__(self, idx):
        ...
        return rf_image, onepw_img, enh_img
...
def main():
  ...
  save_dir = '/mnt/nfs/efernandez/trained_models/UNet_Nair/v1_50epoch'
  ...
  train_dataset = ONEPW_Dataset(TRAIN_PATH, TRAIN_ONEPW_PATH, TRAIN_ENH_PATH)
  train_loader = DataLoader(train_dataset, batch_size = batch_size, shuffle=True)

  nn_model = UNET(residual=True, attention_res=[], group_norm=True).to(device)
  ...
  for ep in range(trained_epochs+1, n_epoch+1):
    for x, y in train_loader:
      ...
      y = y.to(device)
      z = z.to(device)
      score1, score2 = nn_model(x)
      pred2 = torch.round(score2)
      loss = nn.L1Loss()
      cost1 = loss(score1, y)
      cost2 = 1.0 - (dice_coeff(pred2, z)/batch_size)
