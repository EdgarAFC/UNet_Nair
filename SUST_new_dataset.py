...
class DAS_PW(torch.nn.Module):
    ...
    def forward(self, x, modified_reconstruction=True, accumulate=False):
        ...
        # Change regular 75 PW reconstruction based on the angles
        if modified_reconstruction:
            # Indices of the elements you want
            indices = [0, 12, 25, 37, 49, 62, 74]
            # Create a new array with the selected elements
            self.ang_list = torch.tensor(indices, dtype=torch.long, device=device)
            # self.angles = self.angles[indices]
            print("Imprimiendo ang_list", self.ang_list)
            print("Imprimiendo angles", self.angles)
...
def main():
    ...
    for id in range(1, 1001):
        ...
        # image_base = np.load(onepw_dir+simu_name+'.npy' )
