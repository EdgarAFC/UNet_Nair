...
class PlaneWaveData:
    ...
    def validate(self):
        ...
        print(self.idata.ndim)
        print(self.qdata.ndim)
        print('-----')
...
class LoadData_nair20202(PlaneWaveData):
    def __init__(self, h5_dir, simu_name):
        ...
        # g = h5py.File(filename, "r")
            ...
            self.idata = np.array(f["signal"], dtype="float32")
            self.qdata = np.imag(hilbert(self.idata, axis=-1))
            print(self.idata.shape)
            print(self.qdata.shape)
            print('--------------------')
            # self.angles = np.array([0])
            self.angles = np.array(np.linspace(-16, 16, 75))
            print('DIM ANGLES', self.angles.size)
            ...
            self.time_zero = np.array(f['time_zero']).flatten()
            print('ZERO DIM',self.time_zero.ndim)
            print('ZERO',self.time_zero)
            self.fdemod = 0
...
def downsample_channel_data2(H, laterals, depths, device):
    norm_value = np.max((np.abs(H.idata), np.abs(H.qdata)))
    H.idata = H.idata / norm_value
    H.qdata = H.qdata / norm_value
    # 0.002, 0.052
    ...
    # Accediendo al primer tensor de tamaño (1, 800, 128)
    x_i = x[:, 0, :, :]

    # Accediendo al segundo tensor de tamaño (1, 800, 128)
    x_q = x[:, 1, :, :]
    return x_i,x_q
...
def delay_plane(grid, angles):
    ...
    dist = x * torch.sin(angles*PI/180) + z * torch.cos(angles*PI/180)
...
class DAS_PW2(torch.nn.Module):
    ...
    def forward(self, x, laterals, depths, accumulate=False):
        """ Forward pass for DAS_PW neural network. """
        dtype, device = self.dtype, self.device
        ...
        idata, qdata = x
        # idata,qdata = downsample_channel_data2(copy.deepcopy(self.P),
        #                                            laterals=laterals,
        #                                            depths=depths,
        #                                            device=device)
        idata = torch.tensor(idata, dtype=dtype, device=device)
        ...
        for i, tx in enumerate(self.ang_list):
            print('TX', tx)
            print('SELF_ANGLES',self.angles[[tx]])
            txdel[i] = delay_plane(self.grid, self.angles[[tx]])
            # txdel[i] += self.time_zero[tx] * self.c   # ORIGINAL
            txdel[i] -= self.time_zero[tx] * self.c     # IT HAS TO BE "-"
            # txapo[i] = apod_plane(self.grid, self.angles[tx], xlims)
...
def make_bimg_das1(h5_dir, simu_name, device):
    ...
    # h5_name = "inclusion_" + inclusion_shape + ".h5"
    # P = ShapeVariationDataLoader(h5_dir, h5_name,simu_name)
    # end shape
...
def main():
    ...
    # save_dir = '/CODIGOS_TESIS/T2/generated_samples/DAS/gen_square/'
    # save_dir = '/CODIGOS_TESIS/T2/generated_samples/UNET_Difusiva/v1_380epoch/gen_triangle/'
    # save_dir = '/CODIGOS_TESIS/T2/generated_samples/Unet_Nair_ONEPW/gen_shape_T/'
    # save_dir = '/CODIGOS_TESIS/T2/generated_samples/Unet_Nair/gen_att/'
    # save_dir = '/CODIGOS_TESIS/T2/generated_samples/DDPM_model/NEW_DATASET/gen_att/'

    # if not os.path.exists(save_dir):
    #     os.mkdir(save_dir)

    # model_dir='/CODIGOS_TESIS/T2/trained_models/Unet_Nair/'
    # model_dir = '/CODIGOS_TESIS/T2/trained_models/DDPM_model/NEW_DATASET/'
    ...
    #     print(simu_name)
    #     bmode, grid_full=create_phantom_shape_diff(shape_dir,h5name,simu_name, depth_ini, device, model,diffusion)
    #     # bmode, grid_full=create_phantom_shape_conv(shape_dir,h5name,simu_name, depth_ini, device, model)
    ...
    #     # bmode, _ = make_bimg_das1(shape_dir, simu_name, device=device)
    ...
    for simu in dir_test:
        ...
        # plt.subplot(1, 3, 1)
        # plt.imshow(bmode_cliped, cmap="gray", vmin=-60, vmax=0, extent=extent, origin="upper")
        # # plt.colorbar()
        # plt.title('Target')
        ...
        # plt.subplot(1, 3, 3)
        # plt.imshow(bmode2, cmap="gray", vmin=-60, vmax=0, extent=extent, origin="upper")
        # # plt.colorbar()
        # plt.title('Adapted UNet')
